...
export default function WaveformWidget() {
  ...
  const { analyserRef } = usePitchDetection();

  const [sensitivity, setSensitivity] = useState(50);
  const [waveformColor, setWaveformColor] = useState('#4ade80');
  const [backgroundColor, setBackgroundColor] = useState('#1f2937');
  const [maxHistory, setMaxHistory] = useState(400);
  ...
  useEffect(() => {
    ...
    const updateAmplitude = () => {
      if (!analyserRef.current) {
        // If analyser isn't ready yet, try again in next frame
